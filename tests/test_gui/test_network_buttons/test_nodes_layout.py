...
def nodes_layout() -> NodesLayout:
    node_set = MagicMock()
    node_set.litecoin.network = MAINNET
    nodes_layout = NodesLayout(node_set)
    return nodes_layout


class TestNodesLayout(object):
    def test_litecoin_qt_button(self, nodes_layout: NodesLayout, qtbot: QTest):
        qtbot.mouseClick(nodes_layout.litecoin_qt_button, Qt.LeftButton)
        nodes_layout.node_set.litecoin.launch.assert_called_once()
